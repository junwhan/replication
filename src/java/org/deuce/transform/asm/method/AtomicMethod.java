...
public class AtomicMethod extends MethodAdapter implements Opcodes{
	...
	public static boolean foo(Object s) throws IOException{
		...
		for( int i=10 ; i>0 ; --i)
		{
			...
			{
				if( context.commit()){
					ContextDelegator.releaseInstance(context); // talex
					if( throwable != null)
	...
	public void visitCode() {
		...
		// talex: Begin modification
		// ContextDelegator.releaseInstance(context);
		// Q: What is l19 for? (seems unused)
		mv.visitVarInsn(ALOAD, contextIndex);
		mv.visitMethodInsn(INVOKESTATIC, ContextDelegator.CONTEXT_DELEGATOR_INTERNAL, 
			"releaseInstance", "(Lorg/deuce/transaction/AbstractContext;)V");
		// talex: End modification
